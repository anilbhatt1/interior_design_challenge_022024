...
$ accelerate launch '/interior_design_challenge_022024/terrain-diffusion-interior-combined-image/scripts/train_text_to_image_lora_sd2_inpaint.py' \
  ...
  --report_to="wandb" \
  --image_dir1="/combined_images/" \
  --image_dir2="/combined_broaden_mask/" \
  --image_dir3="/combined_image_mask_broaden/" \
  --caption_file_path='/content/combined_json.json' \
...
class InpaintingDataset(Dataset):
    def __init__(self, mask_names, caption_dict, tokenizer, img_dir1, img_dir2, img_dir3, transform=None):
        self.data = mask_names
        self.caption_dict = caption_dict
        self.tokenizer = tokenizer
        self.data_collator = DataCollatorWithPadding(tokenizer=self.tokenizer)
        self.img_dir = img_dir1
        self.mask_dir = img_dir2
        self.img_mask_dir = img_dir3
...
def prepare_img_captions(caption_file_path, mask_dir):
    ...
    mask_names_lst = os.listdir(mask_dir)
    caption_dict = {}
    for image_name, caption in caption_data.items():
        caption_dict[image_name] = caption
    print(f' caption lengths : {len(mask_names_lst)}, {len(caption_dict)}')

    return mask_names_lst, caption_dict
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Location of ikea images directory",
    )      
    parser.add_argument(
        "--image_dir2",
        type=str,
        default='/ikea_mask_broaden/',
        help="Location of ikea mask images directory",
    ) 
    parser.add_argument(
        "--image_dir3",
        type=str,
        default='/ikea_image_mask_broaden/',
        help="Location of ikea images-with-mask directory",
    )       
    parser.add_argument(
        "--val_image_dir",
        type=str,
        default='/val_images_rose/',
...
def main():
    ...
    mask_names_lst, caption_dict = prepare_img_captions(args.caption_file_path, args.image_dir2)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        if accelerator.is_main_process:
            if args.validation_file is not None and epoch % args.validation_epochs == 0:
                ...
                print(f'About to start validation : {val_examples}')
                ...
                for tracker in accelerator.trackers:
                    ...
                    if tracker.name == "wandb":
                        print(f' Saving val images to wandb')
                        tracker.log(
                            {
                                "validation": [
                                    wandb.Image(
    ...
    # Save the lora layers
    accelerator.wait_for_everyone()
    print(f'Final float32 save Lora layers in {args.output_dir}')
    if accelerator.is_main_process:
        unet = unet.to(torch.float32)
        unet.save_attn_procs(args.output_dir)
        print(f'Lora layers saved in float32 {args.output_dir} on epoch {epoch}!!')
